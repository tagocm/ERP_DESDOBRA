...
type SubmitCorrectionLetterInput = {
    accessKey: string;
    sequence: number;
...
type SubmitCorrectionLetterResult = {
    success: boolean;
    cStat: string;
    xMotivo: string;
    eventCStat?: string;
...
function parseEventResponse(xml: string): {
    ...
    if (!result?.cStat) {
        throw new NfeSefazError("Resposta da SEFAZ inválida para CC-e.", "PARSE", { xml });
    }
...
export async function submitCorrectionLetter(
    input: SubmitCorrectionLetterInput,
    cert: SefazCertConfig,
    options?: SefazRequestOptions
): Promise<SubmitCorrectionLetterResult> {
    const sequence = String(input.sequence).padStart(2, "0");
    const eventId = `ID110110${input.accessKey}${sequence}`;
    ...
    const unsignedEventXml =
        ...
        `<xCorrecao>${escapeXml(input.correctionText)}</xCorrecao>` +
        `<xCondUso>${escapeXml(CCE_USAGE_CONDITIONS)}</xCondUso>` +
        `</detEvento>` +
        `</infEvento>` +
        `</evento>` +
        `</envEvento>`;
    ...
    if (status !== 200) {
        throw new NfeSefazError(`Erro HTTP ao enviar CC-e: ${status}`, "HTTP", { body });
    }

    const parsed = parseEventResponse(body);
    const eventCStat = parsed.eventCStat || parsed.envelopeCStat;
    const eventXMotivo = parsed.eventXMotivo || parsed.envelopeXMotivo;
    const isSuccess = parsed.envelopeCStat === "128" && (eventCStat === "135" || eventCStat === "136");
