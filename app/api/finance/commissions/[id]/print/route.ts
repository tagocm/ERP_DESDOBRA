...
async function resolveRepresentativeProfile(
  supabase: Awaited<ReturnType<typeof resolveCompanyContext>>["supabase"],
  companyId: string,
  repId: string,
...
export async function GET(
  ...
  try {
    ...
    const { data: settlementData, error: settlementError } = await supabase
      ...
      .select("id, document_number, rep_id, cutoff_date, status, total_paid, created_at")
      .eq("company_id", companyId)
      .eq("id", settlementId)
      .single();
    ...
    if (settlementError || !settlementData) {
      return NextResponse.json(
        { error: settlementError?.message ?? "Acerto não encontrado." },
        { status: 404 },
      );
    }
    ...
    if (itemsError) {
      throw new Error(`Falha ao carregar itens do acerto: ${itemsError.message}`);
    }

    const settlementItems = ((itemsData ?? []) as SettlementItemRow[]).filter(
    ...
    const releaseRowsPromise = releaseIds.length > 0
      ? supabase
    ...
    const [releaseRowsResult, paymentEntryResult, settingsResult] = await Promise.all([
      releaseRowsPromise,
      supabase
    ...
    const orders = Array.from(contributionsByOrderId.values())
      .map((contribution) => {
    ...
    const html = renderCommissionSettlementA4Html({
      company: {
    ...
    const fileName = `acerto_comissao_${displayNumber}.pdf`;
    ...
    const message = error instanceof Error ? error.message : "Erro inesperado ao gerar PDF do acerto de comissão.";
    logger.error("[finance/commissions/print] failed generating settlement PDF", { message });
