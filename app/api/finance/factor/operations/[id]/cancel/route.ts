...
import { cancelFactorOperationSchema } from "@/lib/services/factor/schemas";
...
export async function POST(
    ...
    try {
        ...
        const parsed = cancelFactorOperationSchema.safeParse(payload);
        if (!parsed.success) {
            return errorResponse("Payload inválido", 400, "INVALID_PAYLOAD", parsed.error.flatten());
        }

        const operation = await service.cancelOperation(params.data.id, parsed.data);
