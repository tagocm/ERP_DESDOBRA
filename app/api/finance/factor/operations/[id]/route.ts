...
import { updateFactorOperationSchema } from "@/lib/services/factor/schemas";
...
export async function PATCH(
    ...
    try {
        ...
        const parsed = updateFactorOperationSchema.safeParse(payload);
        if (!parsed.success) {
            return errorResponse("Payload inválido", 400, "INVALID_PAYLOAD", parsed.error.flatten());
        }

        const operation = await service.updateOperation(params.data.id, parsed.data);
