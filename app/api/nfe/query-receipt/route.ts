...
export async function POST(request: NextRequest) {
    try {
        ...
        // Legacy fallback metadata (for branches still using sales_document_nfes)
        let legacyNfe: {
        ...
        if (!emission) {
            const { data: legacy } = await supabaseAdmin
                ...
                .select('document_id, nfe_number, nfe_series')
                .eq('company_id', companyId)
        ...
        if (!safeNumero || !safeSerie) {
            return errorResponse(
                "Emissão NF-e não encontrada para esta chave. Refaça a emissão pelo pedido para sincronizar os dados.",
                404,
                "EMISSION_NOT_FOUND"
            );
        }

        // Upsert (Create if missing, with valid fiscal identity fields)
