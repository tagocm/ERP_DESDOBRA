...
export default function ProductionSectorsPage() {
  ...
  const saveSector = async () => {
    ...
    const normalizedCode = normalizeCode(formCode)
    ...
    try {
      if (editingSector) {
        const { error } = await supabase
          .from('production_sectors')
          .update({ code: normalizedCode, name: normalizedName, is_active: formActive })
          .eq('company_id', selectedCompany.id)
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <Card>
        <CardHeaderStandard
          ...
          description="Cadastre e mantenha setores para classificar OPs de produção e envase."
          actions={
      ...
      <Dialog open={isModalOpen} onOpenChange={(open) => !open && closeModal()}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Informe código, nome e status do setor de produção.</DialogDescription>
          </DialogHeader>
