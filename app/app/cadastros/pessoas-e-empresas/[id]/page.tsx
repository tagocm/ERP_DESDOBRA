...
import { Loader2, Search, CheckCircle2, Save, ArrowLeft, ShoppingCart, Package, Truck, FileText, Receipt, Users, MessageSquare, Settings } from "lucide-react";
...
export default function EditOrganizationPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            try {
                // 1. Organization
                const org = await getOrganizationById(supabase, selectedCompany.id, id);

                if (!org) {
                    throw new Error("Organização não encontrada (retornou null).");
                }

                setFormData({
                    document_number: org.document_number ? formatCNPJ(org.document_number) : "",
                ...
                setCommercialData({
                    price_table_id: org.price_table_id || "",
                    default_payment_terms_days: org.default_payment_terms_days ? String(org.default_payment_terms_days) : "",
                    sales_rep_user_id: org.sales_rep_user_id || "",
                    freight_terms: (['cif', 'fob', 'retira'].includes(org.freight_terms || '') ? org.freight_terms : "") as '' | 'cif' | 'fob' | 'retira',
                    notes_commercial: org.notes_commercial || "",

                    // New Fields
                    credit_limit: org.credit_limit ? String(org.credit_limit) : "",
                    default_discount: org.default_discount ? String(org.default_discount) : "",
                    sales_channel: org.sales_channel || "",
                    payment_terms_id: org.payment_terms_id || "",
                    purchase_payment_terms_id: org.purchase_payment_terms_id || "",
                    delivery_terms: org.delivery_terms || "",
                    lead_time_days: org.lead_time_days ? String(org.lead_time_days) : "",
                    minimum_order_value: org.minimum_order_value ? String(org.minimum_order_value) : "",
                    preferred_carrier_id: org.preferred_carrier_id || "",
                    region_route: org.region_route || "",
                    payment_mode_id: org.payment_mode_id || ""
                });

                setFiscalData({
                    is_simple_national: org.is_simple_national,
                    is_public_agency: org.is_public_agency,
                    ie_indicator: org.ie_indicator,
                    state_registration: org.state_registration || "",
                    municipal_registration: org.municipal_registration || "",
                    suframa: org.suframa || "",
                    email_nfe: org.email_nfe || "",

                    // New Fields mapping
                    tax_regime: org.tax_regime || (org.is_simple_national ? "Simples Nacional" : ""),
                    icms_contributor: (org.ie_indicator === 'exempt' ? 'Isento' : org.ie_indicator === 'non_contributor' ? 'Não contribuinte' : 'Contribuinte'),
                    is_ie_exempt: org.is_ie_exempt || false,
                    is_final_consumer: org.is_final_consumer || false,
                    public_agency_sphere: org.public_agency_sphere || "",
                    public_agency_code: org.public_agency_code || "",
                    default_operation_nature: org.default_operation_nature || "",
                    default_cfop: org.default_cfop || "",
                    notes_fiscal: org.notes_fiscal || "",
                });
                ...
                // 3. Addresses
                const addresses = await getAddresses(supabase, selectedCompany.id, id);
                const billing = addresses.find(a => a.type === 'billing' || a.is_default) || addresses[0];
                if (billing) {
                    setBillingAddress({
                        id: billing.id, // Keep ID for update
                        zip: billing.zip || "",
                ...
                // 4. Contacts
                const people = await getPeople(supabase, selectedCompany.id, id);
                setContacts(people.map((p: any) => ({
                    id: p.id,
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            // Validation Logic duplicated for now
            if (cleanDoc) {
                if (!validateCNPJ(sanitizedFormData.document_number) && cleanDoc.length !== 11) {
                    if (cleanDoc.length === 14) throw new Error("CNPJ inválido");
                }
            }

            // Update Organization
            await updateOrganization(supabase, id, {
                ...
                // Derive is_simple_national from tax_regime
            ...
            await setOrganizationRoles(supabase, selectedCompany.id, id, selectedRoles);

            // Update Address
            if (sanitizedAddress.zip || sanitizedAddress.city) {
                await upsertAddress(supabase, {
                    id: sanitizedAddress.id, // Pass ID if updating existing
            ...
            router.push("/app/cadastros/pessoas-e-empresas?success=updated");
    ...
    return (
        <div>
            ...
            <PageHeader
                title={formData.trade_name || "Editando Cadastro"}
                subtitle="Cadastre clientes, fornecedores e parceiros."
                actions={
                    <div className="flex gap-2">
                        <Button variant="secondary" onClick={() => router.push('/app/cadastros/pessoas-e-empresas')} disabled={isLoading || isFetching}>
                            Cancelar
                        </Button>
