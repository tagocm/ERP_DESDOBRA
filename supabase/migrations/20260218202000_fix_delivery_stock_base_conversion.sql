-- Fix: route stock deduction must use base quantity (qty_base), not display quantity.
-- Example: 2 fardos de 12 UN devem gerar saída de 24 UN.
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            ...
            -- Convert loaded display qty to base qty using order item ratio (qty_base / quantity)
            v_qty_base := COALESCE(
                CASE
                    WHEN COALESCE(r_item.order_qty_display, 0) > 0 AND r_item.order_qty_base IS NOT NULL
                        THEN COALESCE(r_item.qty_loaded, 0) * (r_item.order_qty_base / r_item.order_qty_display)
                    ELSE COALESCE(r_item.qty_loaded, 0)
                END,
                0
            );
...
-- Backfill/correction for existing delivery_item stock outs:
-- align movement qty with converted base quantity.
WITH expected AS (
    SELECT
        im.id AS movement_id,
        COALESCE(
            CASE
                WHEN COALESCE(sdi.quantity, 0) > 0 AND sdi.qty_base IS NOT NULL
                    THEN COALESCE(di.qty_loaded, 0) * (sdi.qty_base / sdi.quantity)
                ELSE COALESCE(di.qty_loaded, 0)
            END,
            0
