...
-- A unicidade de nome não deve ser global por empresa.
-- Regra correta: impedir nomes iguais apenas dentro da mesma pasta (parent_id).
DROP INDEX IF EXISTS public.idx_financial_categories_name_company;

CREATE INDEX IF NOT EXISTS idx_financial_categories_company_name
...
BEGIN
  ...
  -- Regra de negócio: nome só pode repetir se estiver em pasta diferente.
  IF EXISTS (
    SELECT 1
    FROM public.gl_accounts sibling
    WHERE sibling.company_id = p_company_id
      AND sibling.parent_id = p_parent_account_id
      AND sibling.is_active IS TRUE
      AND lower(btrim(sibling.name)) = lower(v_name)
  ) THEN
    RAISE EXCEPTION 'Já existe uma conta com este nome nesta mesma subcategoria.' USING ERRCODE = 'P0001';
  END IF;
  ...
  SELECT COALESCE(MAX(split_part(code, v_parent_code || '.', 2)::integer), 0)
    INTO v_max_existing_suffix
  FROM public.gl_accounts
  WHERE company_id = p_company_id
    AND parent_id = p_parent_account_id
    AND code LIKE v_parent_code || '.%'
    AND split_part(code, v_parent_code || '.', 2) ~ '^[0-9]+$';
  ...
  LOOP
    ...
    EXIT WHEN NOT EXISTS (
      SELECT 1
      FROM public.gl_accounts existing_code
      WHERE existing_code.company_id = p_company_id
        AND existing_code.code = v_account_code
    );
...
EXCEPTION
  WHEN unique_violation THEN
    IF position('gl_accounts_code_company_unique' IN SQLERRM) > 0 THEN
...
BEGIN
  ...
  ON CONFLICT (company_id, expense_account_id) WHERE expense_account_id IS NOT NULL DO NOTHING;
