-- Fix: robust conversion fallback for stock deduction from deliveries.
-- Priority for conversion factor:
-- 1) sales_document_items.qty_base / quantity
-- 2) sales_document_items.conversion_factor_snapshot
-- 3) item_packaging.qty_in_base
-- 4) 1
    ...
    LOOP
        ...
        FOR r_item IN
            SELECT
                ...
                sdi.packaging_id,
                pkg.qty_in_base AS packaging_qty_in_base,
                i.avg_cost
              FROM public.delivery_items di
              JOIN public.sales_document_items sdi ON sdi.id = di.sales_document_item_id
              LEFT JOIN public.item_packaging pkg ON pkg.id = sdi.packaging_id
        ...
        LOOP
            ...
            v_factor := COALESCE(
                ...
                NULLIF(r_item.packaging_qty_in_base, 0),
                1
            );
...
WITH expected AS (
    ...
    LEFT JOIN public.item_packaging pkg
      ON pkg.id = sdi.packaging_id
