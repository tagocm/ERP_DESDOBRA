...
DECLARE
    ...
    v_outbound_key text;
    v_existing record;
    v_existing_inbound_status text;
    v_has_active_job boolean := false;
    v_can_reprocess boolean := false;
    v_mode text;
    v_reason_code text;
    v_reason_other text;
...
BEGIN
    ...
    SELECT nir.id, nir.status, nir.inbound_emission_id
      INTO v_existing
    ...
    IF v_existing.id IS NOT NULL AND v_existing.inbound_emission_id IS NOT NULL THEN
        SELECT ne.status
          INTO v_existing_inbound_status
        FROM public.nfe_emissions ne
        WHERE ne.id = v_existing.inbound_emission_id
        LIMIT 1;
    END IF;
    ...
    IF v_existing.id IS NOT NULL THEN
        SELECT EXISTS (
            ...
            WHERE jq.job_type = 'NFE_INBOUND_REVERSAL_EMIT'
              AND jq.status IN ('pending', 'processing')
              AND COALESCE(jq.payload->>'reversalId', '') = v_existing.id::text
        ) INTO v_has_active_job;
    END IF;

    v_can_reprocess := (
        COALESCE(v_existing.status, '') = 'failed'
        OR COALESCE(v_existing_inbound_status, '') = 'cancelled'
        OR (
            COALESCE(v_existing.status, '') = 'processing'
            AND v_existing.inbound_emission_id IS NULL
            AND NOT v_has_active_job
        )
    );

    IF v_existing.id IS NOT NULL AND NOT v_can_reprocess THEN
    ...
    IF v_existing.id IS NOT NULL AND v_can_reprocess THEN
        UPDATE public.nfe_inbound_reversals
        SET
            outbound_emission_id = p_outbound_emission_id,
            outbound_sales_document_id = v_outbound.sales_document_id,
            mode = v_mode,
            reason_code = v_reason_code,
            reason_other = v_reason_other,
            internal_notes = v_internal_notes,
            selection = v_selection,
            inbound_emission_id = NULL,
            status = 'pending',
            c_stat = NULL,
            x_motivo = NULL,
            updated_at = NOW()
    ...
    INSERT INTO public.audit_logs (
        ...
        p_company_id,
        p_created_by,
        CASE WHEN v_can_reprocess THEN 'NFE_ENTRADA_ESTORNO_REENFILEIRADA' ELSE 'NFE_ENTRADA_ESTORNO_SOLICITADA' END,
        'nfe',
