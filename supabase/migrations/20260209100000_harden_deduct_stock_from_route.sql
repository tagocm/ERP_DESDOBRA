-- Harden deduct_stock_from_route (multi-tenant + spoofing)
-- - Enforce route tenant membership for authenticated callers
-- - Prevent spoofing created_by (p_user_id must match auth.uid when present)
-- - Preserve existing behavior for service_role/background usage
-- - Set search_path for SECURITY DEFINER safety
...
-- Keep explicit least-privilege grants (idempotent)
