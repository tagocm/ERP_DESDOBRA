...
DECLARE
  ...
  v_sum numeric(14,2);
BEGIN
  SELECT ai.company_id, ai.amount_original::numeric(14,2)
    INTO v_company_id, v_target_amount
    FROM public.ar_installments ai
   WHERE ai.id = p_installment_id;
  ...
  SELECT COALESCE(SUM(parsed.amount), 0)
    INTO v_sum
    FROM parsed;
  ...
  DELETE FROM public.ar_installment_allocations allocation
...
BEGIN
  ...
  DELETE FROM public.ap_installment_allocations allocation
