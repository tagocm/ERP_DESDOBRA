...
interface FuelRecordsToolbarProps {
    search: string;
...
export function FuelRecordsToolbar({
    search,
    onSearchChange,
    fuelType,
    onFuelTypeChange,
    ...
    endDate,
    onEndDateChange,
}: FuelRecordsToolbarProps) {
    const hasFilters = search || fuelType !== 'all' || startDate || endDate;
            ...
            {hasFilters && (
                <div className="flex items-center gap-2 animate-in fade-in slide-in-from-top-1 duration-200">
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        {fuelType !== 'all' && (
                            <Badge variant="secondary" className="bg-brand-50 text-brand-700 hover:bg-brand-100 border-none pl-2 pr-1 h-6 rounded-2xl font-medium text-[11px]">
                                Tipo: {fuelTypeLabels[fuelType as keyof typeof fuelTypeLabels]}
                                <button onClick={() => onFuelTypeChange('all')} className="ml-1 p-0.5 hover:bg-brand-200 rounded-full transition-colors">
                                    <X className="w-3 h-3" />
                                </button>
                            </Badge>
                        )}
