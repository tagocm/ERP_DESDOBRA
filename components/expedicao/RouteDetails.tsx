...
export function RouteDetails({ route, onClose, onStartRoute }: RouteDetailsProps) {
    ...
    const countLoaded = route.orders?.filter((o: any) => o.loading_status === 'loaded' || (!o.loading_status && o.sales_order?.loading_checked)).length || 0;
    const countPartial = route.orders?.filter((o: any) => o.loading_status === 'partial').length || 0;
    const countNotLoaded = route.orders?.filter((o: any) => o.loading_status === 'not_loaded' || (o.loading_status === 'pending' && o.partial_payload?.status === 'not_loaded')).length || 0;
    const totalCount = route.orders?.length || 0;

    // Effective pending: real pending ones (no status decision yet)
    const countProcessed = countLoaded + countPartial + countNotLoaded;
